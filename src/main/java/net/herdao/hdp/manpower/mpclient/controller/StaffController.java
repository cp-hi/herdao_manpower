...
//    @PreAuthorize("@pms.hasPermission('mpclient_staff_view')" )
    public R getStaffCount(Long groupId,Long orgId) {
        return R.ok(staffService.queryCount(groupId,orgId));
    }
...
//    @PreAuthorize("@pms.hasPermission('mpclient_staff_del')" )
    ...
    @ApiOperation(value = "人事异动(员工)基础信息-多页面通用（根据id查询基本信息）")
    @GetMapping("/changes/basic")
    public R<StaffBasicVO> getStaffBasic(@RequestParam("id") Long id) throws Exception {
        return R.ok(staffService.selectBasicById(id));
    }
